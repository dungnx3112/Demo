...
static void compute_softmax(hls::stream<float>& in_stream,
  ...
  float vec_local_1[kVocabSize];  // Use kVocabSize as max buffer size
  float vec_local_2[kVocabSize];

  if (vec_size == -1) {
    in_max_idx = kSeqLen;  // Use kSeqLen as default
  }
  ...
  float sum = 0;
  for (int i = 0; i < in_max_idx; i++) {
    vec_local_2[i] = expf(vec_local_1[i] - max_val);  // Use expf for HLS
    sum += vec_local_2[i];
  }
