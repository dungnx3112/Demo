...
static void compute_rope(hls::stream<float>& q_in_stream,
  ...
  float cos_local[kHalvedHeadDim];
  float sin_local[kHalvedHeadDim];

  float q_out_local[kDim];
  float k_out_local[kDim];
...
#pragma HLS dataflow
  load_vec(q_in, q_in_stream, kDim);
  load_vec(k_in, k_in_stream, kDim);
  load_vec(cos_vec, cos_vec_stream, kHalvedHeadDim);
  load_vec(sin_vec, sin_vec_stream, kHalvedHeadDim);
  compute_rope(q_in_stream, k_in_stream, cos_vec_stream, sin_vec_stream,
               q_out_stream, k_out_stream, head_begin);
  store_result(q_out, q_out_stream, kDim);
  store_result(k_out, k_out_stream, kDim);
}
}
